...
Python_Libraries_Updatergui = None
fmod = {}
...
"""
	...
	if len(data) > 0:
		...
		Python_Libraries_Updatergui.infolab.config(text="No updates found!")
...
def install_module(module):
	...
	if Python_Libraries_Updatergui.usermode:
		res = subprocess.run([Python_Libraries_Updatergui.pip, "install", "--user", module.get()], stdout=subprocess.PIPE)
	else:
		res = subprocess.run([Python_Libraries_Updatergui.pip, "install", module.get()], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")
...
def search_module(module):
	...
	res = subprocess.run([Python_Libraries_Updatergui.pip, "search", module.get()], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")
	#tkinter.messagebox.showinfo(title="Result", message=output)
	r = tkinter.Tk()
	lb = tkinter.Label(r, text=output, justify="left")
...
def uninstall():
	mod = Python_Libraries_Updatergui.modules.curselection()[0]
	mod += 1
	if tkinter.messagebox.askokcancel(title=f"Uninstall {fmod[mod][0]}", message=f"{fmod[mod][0]} {fmod[mod][1]} will be COMPLETELY uninstalled."):
		res = subprocess.run([Python_Libraries_Updatergui.pip, "uninstall", "-y", fmod[mod][0]], stdout=subprocess.PIPE)
		output = str(res.stdout,"latin-1")
		Python_Libraries_Updatergui.modules.delete(mod-1)
		r = tkinter.Tk()
...
def update():
	...
	if tkinter.messagebox.askokcancel(title=f"Update {fmod[mod][0]}", message=f"{fmod[mod][0]} will be updated from {fmod[mod][1]} to {fmod[mod][2]}"):
		if Python_Libraries_Updatergui.usermode:
			res = subprocess.run([Python_Libraries_Updatergui.pip, "install", "--upgrade", fmod[mod][0]], stdout=subprocess.PIPE)
		else:
			res = subprocess.run([Python_Libraries_Updatergui.pip, "install", "--upgrade", "--user", fmod[mod][0]], stdout=subprocess.PIPE)
		output = str(res.stdout,"latin-1")
		Python_Libraries_Updatergui.modules.delete(mod-1)
		r = tkinter.Tk()
		lb = tkinter.Label(r, text=output, justify="left")
...
def onselect(evt):
	...
	# value = w.get(index)
	try:
		Python_Libraries_Updatergui.infolab.config(text=fmod[index][0]+" - Current Version: "+fmod[index][1]+" - PIP Version: "+fmod[index][2])
	except:
		Python_Libraries_Updatergui.infolab.config(text=fmod[index][0]+" "+fmod[index][1])		

def reconnect():
	if internet():
		Python_Libraries_Updatergui.b_updatecheck.config(state="normal")
		Python_Libraries_Updatergui.b_install.config(state="normal")
		Python_Libraries_Updatergui.b_rec.destroy()
		Python_Libraries_Updatergui.online = True
		Python_Libraries_Updatergui.infolab.config(text="Reconnected to network!")
	else:
		tkinter.messagebox.showerror(title="No network connection", message="No internet connection was found.\nPython_Libraries_Updater will run in offline mode. (No update checking.)")	

def pipcheck():
	res = subprocess.run([Python_Libraries_Updatergui.pip, "check"], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")	
	r = tkinter.Tk()
	lb = tkinter.Label(r, text=output)
	lb.grid()
...
def pipshow():
	try:
		...
		return
	mod += 1
	res = subprocess.run([Python_Libraries_Updatergui.pip, "show", fmod[mod][0]], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")
	r = tkinter.Tk()
	lb = tkinter.Label(r, text=output, justify="left")
	lb.grid()
...
def about():
	tkinter.messagebox.showinfo(title="About Python_Libraries_Updater", message="Python_Libraries_Updater is a pip GUI interface written by Kaiser.\nSource is available at https://github.com/Kaiz0r/Python_Libraries_Updater")
...
class pipGuiMan:
	def __init__(self):
		...
		self.mainwin.title("Python_Libraries_Updater")
		...
		self.menu.add_cascade(label="Python_Libraries_Updater", menu=self.filemenu)
...
Python_Libraries_Updatergui = pipGuiMan()	
Python_Libraries_Updatergui.mainwin.mainloop()
print(Python_Libraries_Updatergui.pip)
